...
slides.each do |slide|
  ...
  if hasCanvas.include? slide.attr('id')
    ...
    frames = @doc.xpath("//*[@image=\"#{slide.attr('id')}\"]/*")
    ...
    frameTimings = frames.xpath('./@timestamp').to_a.map(&:to_s).map(&:to_f).each_cons(2).map do |a, b| (b - a).round(1)
    end << (slide.attr('out').to_f - canvasEnd.to_s.to_f).round(1)
    ...
    export = "rsvg-convert --format=png --output=slides/slide#{slideNumber}.png tmp.svg"
    system(export)
    ...
    open("slides/slide#{slideNumber}.png", 'wb') do |file|
